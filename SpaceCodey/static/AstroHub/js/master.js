...
function giveBirth(e, u) {
  ...
  var mouseX = e.clientX - rect.left; // Calculate mouse X position relative to canvas
  var mouseY = e.clientY - rect.top; // Calculate mouse Y position relative to canvas

  for (var i = 0; i < u; i++) {
    var newFirefly = new Firefly(persons.length);
    newFirefly.x = mouseX;
    newFirefly.y = mouseY;
    persons.push(newFirefly);
  }
}